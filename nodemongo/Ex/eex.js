...
// MongoClient.connect(url,{ useUnifiedTopology: true } , function(err, db) {
//   if (err) throw err;
//   var dbo = db.db("music3");
//   dbo.createCollection("songdetails​3", function(err, res) {
//     if (err) throw err;
...
//   ];
//   dbo.collection("songdetails​3").insertMany(details, function(err, res) {
//     if (err) throw err;
...
MongoClient.connect(url,{ useUnifiedTopology: true } ,  function(err, db) {
  if (err) throw err;
  var dbo = db.db("music3");
  dbo.collection("songdetails3").find({}).toArray(function(err, result) {
    if (err) throw err;
    console.log(result);
    db.close();
  });
